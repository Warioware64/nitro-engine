...
{
	b3ArenaSharedState* shared = arena->shared;
	// See the note on this function in arena_allocator.h: this path is kept
	// rather than refused because it is self-correcting, but it still means
	// the arena was sized too small for what the step actually needed.
	b3Log( "arena overflow: %d bytes beyond capacity %d (will grow on sync)", size, arena->capacity );
	B3_ASSERT( false );
