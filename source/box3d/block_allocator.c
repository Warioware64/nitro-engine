...
b3BlockAllocator b3CreateBlockAllocator( int elementSize, int initialCount )
{
	...
	// Granularity, chosen per allocator rather than globally -- see the header.
	// The requested count rounded up to a power of two, capped at the global
	// maximum. An allocator asked for nothing gets the smallest granularity
	// there is, so a size class nobody expected to be used costs two elements
	// if it ever is, not thirty-two.
	int blockExponent = b3BoundingPowerOf2( initialCount );
	allocator.blockExponent = blockExponent < B3_BLOCK_EXPONENT ? blockExponent : B3_BLOCK_EXPONENT;
	...
	if ( initialCount > 0 )
	{
		int blockSize = 1 << allocator.blockExponent;
		int count = ( initialCount + blockSize - 1 ) >> allocator.blockExponent;
		b3Array_Resize( allocator.blocks, count );

		for ( int i = 0; i < allocator.blocks.count; ++i )
		{
			allocator.blocks.data[i].memory = b3Alloc( blockSize * elementSize );
		}
	}
...
{
	int blockBytes = ( 1 << allocator->blockExponent ) * allocator->elementSize;

	for ( int i = 0; i < allocator->blocks.count; ++i )
	{
		b3Free( allocator->blocks.data[i].memory, blockBytes );
	}
...
void* b3AllocateElement( b3BlockAllocator* allocator )
{
	...
	int blockExponent = allocator->blockExponent;
	int requiredBlockCount = ( index >> blockExponent ) + 1;
	...
	if ( requiredBlockCount > allocator->blocks.count )
	{
		...
		{
			allocator->blocks.data[i].memory = b3Alloc( ( 1 << blockExponent ) * allocator->elementSize );
		}
	}

	int blockIndex = index >> blockExponent;
	int blockOffset = index & ( ( 1 << blockExponent ) - 1 );
