...
// The key packs two shape indices and a child index, and must fill the key
// width exactly -- constants.h derives B3_CHILD_POWER from the other two to
// guarantee that.
_Static_assert( 2 * B3_SHAPE_POWER + B3_CHILD_POWER == B3_SHAPE_KEY_BITS, "shape pair key does not fill its width" );

// The child index identifies a child *within a compound shape*: only
// compound.c creates child proxies in the broad phase. Meshes are a single
// proxy with their own internal BVH, so this is not a triangle index and does
// not have to scale with mesh size.
//
// Compounds are outside the ported feature set, so childIndex is always 0
// here. The 8 bits B3_SHAPE_POWER of 12 leaves over would cap a compound at
// 256 children if they were ever re-enabled.
_Static_assert( B3_CHILD_POWER >= 8, "child index field too narrow for compound shapes" );
...
///
/// The 32-bit variant, matching the 32-bit key. Two 32-bit multiplies rather
/// than upstream's two 64-bit ones, which on ARMv5TE would each expand to a
/// multi-instruction sequence.
///
/// A hash of zero marks an empty slot, so the result is forced non-zero.
/// Upstream only asserts this cannot happen; b3NonZeroHash already exists for
/// exactly this purpose and makes it true instead of merely checked.
static inline uint32_t b3KeyHash( b3ShapeKey key )
{
#if B3_SHAPE_KEY_BITS == 32
	uint32_t h = key;
	h ^= h >> 16;
	h *= 0x85ebca6bu;
	h ^= h >> 13;
	...
	return b3NonZeroHash( h );
}

static int32_t b3FindSlot( const b3HashSet* set, b3ShapeKey key, uint32_t hash )
{
...
static void b3AddKeyHaveCapacity( b3HashSet* set, b3ShapeKey key, uint32_t hash )
{
