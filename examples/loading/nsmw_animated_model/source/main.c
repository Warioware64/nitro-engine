...
// This example loads an NSMW model (NitroSkin MultiWeight): an animated mesh
// with up to two bone weights per vertex for smooth skinning. The animation
// data is the regular DSA format shared with DSMA. It also shows blending
// between two animations (NSMW_PrepareNodesBlend) with the A button.

#include <NEAMain.h>

#include "robot_nsmw_bin.h"
...
int main(int argc, char *argv[])
{
    ...
    // Create an NSMW (two-weight smooth skinning) model
    Scene.Model = NEA_ModelCreate(NEA_AnimatedMW);

    NEA_Animation *AnimWalk = NEA_AnimationCreate();
    NEA_AnimationLoad(AnimWalk, robot_walk_dsa_bin);

    NEA_Animation *AnimWave = NEA_AnimationCreate();
    NEA_AnimationLoad(AnimWave, robot_wave_dsa_bin);

    // ------ Load the NSMW model (multi-material container) -------
    NEA_ModelLoadNSMW(Scene.Model, robot_nsmw_bin);

    NEA_ModelSetAnimation(Scene.Model, AnimWalk);
    NEA_ModelAnimStart(Scene.Model, NEA_ANIM_LOOP, floattof32(0.1));

    // Secondary animation, used to demonstrate animation blending
    NEA_ModelSetAnimationSecondary(Scene.Model, AnimWave);
    ...
    // ---- Auto-bind materials to the NSMW submeshes by name ----
    ...
    while (1)
    {
        ...
        if (keys & KEY_A)
        {
            blend += floattof32(0.05);
            if (blend > floattof32(1))
                blend = floattof32(1);
            NEA_ModelAnimSecondarySetFactor(Scene.Model, blend);
        }
        if (keys & KEY_B)
