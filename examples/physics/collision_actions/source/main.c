...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < 6; i++)
    {
        ...
        NEA_PhysicsSetModel(Scene.Physics[i], // Physics object
                           Scene.Model[i]); // Model assigned to it

        NEA_PhysicsEnable(Scene.Physics[i], false);
    ...
    // Object coordinates
    NEA_ModelSetCoord(Scene.Model[0], 0, 4, 1);
    NEA_ModelSetCoord(Scene.Model[1], 0, 4, 0);
    NEA_ModelSetCoord(Scene.Model[2], 0, 4, -1);
    NEA_ModelSetCoord(Scene.Model[3], 0, 0, 1);
    NEA_ModelSetCoord(Scene.Model[4], 0, 0, 0);
    NEA_ModelSetCoord(Scene.Model[5], 0, 0, -1);
    ...
    NEA_PhysicsOnCollision(Scene.Physics[1], NEA_ColStop);
    NEA_PhysicsOnCollision(Scene.Physics[2], NEA_ColNothing);
